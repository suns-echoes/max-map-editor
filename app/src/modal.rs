...
use crate::matcheditor::{self, MatchEditor};
...
pub enum ModalAction {
	...
	StopFix,
	/// Undo the just-applied Fix Shore result (its one undo unit) and reset the
	/// dialog's stats to the reverted coast; the modal stays open.
	UndoFix,
	...
	AbortGenerate,
	/// Roll the generator's Surprise Me values (the shell supplies the map size).
	Surprise,
	...
	FinishWrlImport,
	/// Save the open Edit Match Data modal: apply its edited rules + grouping to
	/// the project packs and write the pack files. The modal stays open.
	SaveMatch,
}
...
pub trait Modal {
	...
	/// Like [`Modal::on_wheel`], but handed the cursor position so a modal with
	/// more than one scroll region (the match-data editor's two tile lists) can
	/// route the wheel to the list under the pointer. Default: ignore the
	/// cursor and fall back to [`Modal::on_wheel`].
	fn on_wheel_at(&mut self, steps: f32, _x: f32, _y: f32, w: f32, h: f32) -> ModalAction {
		self.on_wheel(steps, w, h)
	}
	/// A secondary (right) press inside the modal. The shell routes right-click
	/// here for modals without a focused text field (so the field edit-menu still
	/// wins when a field is focused). Default: swallow it.
	fn on_right_press(&mut self, _x: f32, _y: f32, _w: f32, _h: f32) -> ModalAction {
		ModalAction::Consumed
	}
	...
	/// Whether this modal blocks the editor beneath it. Blocking modals dim the
	/// screen and swallow all input; a non-blocking one (the generator) floats
	/// on top and lets the map / panels keep working when you act outside it.
	fn blocking(&self) -> bool {
		true
	}
	/// The modal's on-screen rectangle (its dialog frame) - decides whether an
	/// event over a non-blocking modal belongs to it or falls through.
	fn bounds(&self, w: f32, h: f32) -> Rect;
...
impl Modal for AutoFix {
	...
	fn on_press(&mut self, x: f32, y: f32, w: f32, h: f32) -> ModalAction {
		match self.on_press(x, y, w, h) {
			// Method selection is internal to the modal.
			autofix::Press::SetMethod(m) => {
				self.method = m;
				ModalAction::Consumed
			}
...
impl Modal for Generator {
	...
	// The generator window floats on top without dimming or blocking the map -
	// you can pan, zoom, and edit while it's open.
	fn blocking(&self) -> bool {
		false
	}
...
fn from_generator(press: generator::Press) -> ModalAction {
	match press {
		...
		generator::Press::Surprise => ModalAction::Surprise,
...
impl Modal for MatchEditor {
	fn on_key(&mut self, key: ModalKey) -> ModalAction {
		match key {
			ModalKey::Escape => ModalAction::Close,
			other => {
				MatchEditor::on_key(self, &other);
				ModalAction::Consumed
			}
		}
	}
	...
	fn on_press(&mut self, x: f32, y: f32, w: f32, h: f32) -> ModalAction {
		let press = MatchEditor::on_press(self, x, y, w, h);
		self.resolve_press(press)
	}

	fn on_release(&mut self, x: f32, y: f32, w: f32, h: f32) -> ModalAction {
		let press = MatchEditor::on_release(self, x, y, w, h);
		self.resolve_press(press)
	}

	fn on_right_press(&mut self, x: f32, y: f32, w: f32, h: f32) -> ModalAction {
		MatchEditor::on_right_press(self, x, y, w, h);
		ModalAction::Consumed
	}

	fn on_drag(&mut self, x: f32, y: f32, w: f32, h: f32) {
		MatchEditor::on_drag(self, x, y, w, h);
	}

	fn on_wheel_at(&mut self, steps: f32, x: f32, y: f32, w: f32, h: f32) -> ModalAction {
		MatchEditor::on_wheel_at(self, steps, x, y, w, h);
		ModalAction::Consumed
	}
...
impl MatchEditor {
	/// Map a [`matcheditor::Press`] to a [`ModalAction`]; Reset is handled in-place
	/// (no project write) so the modal stays open showing the reverted state.
	fn resolve_press(&mut self, press: matcheditor::Press) -> ModalAction {
		match press {
			matcheditor::Press::Consumed => ModalAction::Consumed,
			matcheditor::Press::Close => ModalAction::Close,
			matcheditor::Press::Save => ModalAction::SaveMatch,
			matcheditor::Press::Reset => {
				self.reset();
				ModalAction::Consumed
			}
		}
