...
/// Selected text in an editable field - accent green at 25% opacity.
pub const TEXT_SELECTION: [f32; 4] = [0.058, 1.0, 0.0, 0.25];
...
/// sRGB byte → linear float. The UI composites in linear RGBA, so every colour
/// byte from a palette / theme is converted through here before it's drawn.
