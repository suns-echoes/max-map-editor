...
app/                 the binary
  ...
  src/input.rs       key/mouse bindings (mme.ini [Bindings]/[Mouse])
  src/ttf.rs         hand-rolled TrueType rasterizer (UI font, from max_square.ttf)
...
resources/           runtime data: tile packs (assets/), starter projects,
                     (templates/), palettes, icons, UI skin, and settings:
                     config/mme.ini (shipped defaults) layered under
                     user/config/mme.ini (the user's saved overrides)
